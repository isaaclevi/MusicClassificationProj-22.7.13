...
namespace MusicClassificationAlgotihm
{
    public class Cluster<T> where T: IPoint<T>
    {
        ...
        public Cluster(T centroid, Type ListType= null)
        {
            this.Centroid = centroid;
            this.Points = Activator.CreateInstance(ListType) as ListPoint<T> ?? new ListPoint<T>() ;
        }
        ...
        internal Cluster<T> CalcNewCenter(IDistance<T> df)
        {
            T newCenter = this.Points.getCenterPoint(df);
            Cluster<T> c = new Cluster<T>(newCenter, Points.GetType());
            return c;
        }
