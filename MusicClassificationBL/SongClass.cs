    ...
    public class SongClass : INotifyPropertyChanged 
    {
        ...
        public SongVector GetMusicVector( )
        {
            ...
            try
            {
                ...
                TimeSpan PlayingTime = new TimeSpan(0, 0, 10);
                this.m_AudioFileReader.CurrentTime = new TimeSpan(this.m_AudioFileReader.TotalTime.Ticks / 4);
        ...
        /// </summary>
        /// <returns>return the amlitude wave</returns>
        private SongVector GetMusicAvgwWithCutsVector( TimeSpan PlayingTime)
        {
            ...
            if (this.m_AudioFileReader!=null)
            {
                ...
                Complex[] fftBuffer = new Complex[fftLength];
                float[] AvgVector = new float[fftLength];
                float[] MaxVector = new float[fftLength];
                float[] MinVector = new float[fftLength];
                int fftPos = 0;
                int read = 0;

                ///read the song
                int counter = 0;
                long StartPos = this.m_AudioFileReader.Position;
                TimeSpan StartTime = this.m_AudioFileReader.CurrentTime;
                while (this.m_AudioFileReader.Position < this.m_AudioFileReader.Length && this.m_AudioFileReader.CurrentTime < endTime)
                {
                    read = m_AudioFileReader.Read(Spectrom, 0, fftLength);
                    if (PerformFFT)
                    {
                        for (int i = 0; i < read; i++)
                        {
                            ...
                            if (fftPos >= fftBuffer.Length)
                            {
                                ...
                                for (int j = 0; j < fftLength; j++)
                                {
                                    if(MaxVector[j]<fftBuffer[j].GetAmplitude())
                                    {
                                        MaxVector[j] = fftBuffer[j].GetAmplitude();
                                    }
                                    if (MinVector[j] > fftBuffer[j].GetAmplitude())
                                    {
                                        MinVector[j] = fftBuffer[j].GetAmplitude();
                                    }
                                }
                            }
                        }
                ...
                this.m_AudioFileReader.Position = StartPos;
                this.m_AudioFileReader.CurrentTime = StartTime;
                while (this.m_AudioFileReader.Position < this.m_AudioFileReader.Length && this.m_AudioFileReader.CurrentTime < endTime)
                {
                    read = m_AudioFileReader.Read(Spectrom, 0, fftLength);
                    if (PerformFFT)
                    {
                        for (int i = 0; i < read; i++)
                        {
                            ...
                            if (fftPos >= fftBuffer.Length)
                            {
                                ...
                                for (int j = 0; j < fftLength; j++)
                                {
                                    if (fftBuffer[j].GetAmplitude() >= (MinVector[j] + (MaxVector[j] - MinVector[j]) / 12) && fftBuffer[j].GetAmplitude() <= (MaxVector[j] - (MaxVector[j] - MinVector[j]) / 12))
                                    {
                                        AvgVector[j] += fftBuffer[j].GetAmplitude();
                                    }
                                }
                ...
                for (int i = 0; i < fftLength; i++)
                {
                    AvgVector[i] = AvgVector[i] / counter;
                }
                SongVector sv = new SongVector(AvgVector);
                sv.Song = this;
                this.m_AudioFileReader.CurrentTime = new TimeSpan(0, 0, 0);
